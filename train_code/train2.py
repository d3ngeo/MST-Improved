#This is the version of training with tv loss only 
import torch
...
import datetime
from losses import total_variation_loss  # ✅ Import TV Loss function
...
class Loss_MRAE(nn.Module):
    def forward(self, output, target):
        error = torch.abs(output - target) / torch.abs(target)
...
def main():
    cudnn.benchmark = False  # Ensure deterministic behavior during training
    iteration = 0
    ...
    while iteration < total_iteration:
        ...
        for i, (images, labels) in enumerate(train_loader):
            ...
            # Compute TV Loss
            tv_weight = 0.00000001  # Adjust this weight experimentally
            tv_loss = total_variation_loss(output.detach())  # Prevents unnecessary gradient computation
            loss = mrae_loss + tv_weight * tv_loss
            loss.backward()
            optimizer.step()
            scheduler.step()
