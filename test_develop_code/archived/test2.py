...
# Move loss functions to GPU if available
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
criterion_mrae = criterion_mrae.to(device)
criterion_rmse = criterion_rmse.to(device)
    ...
    with torch.no_grad():  # Save memory during inference
        for i, (input, target) in enumerate(val_loader):
            try:
                ...
                result = np.clip(result, 0, 1)  # Clip values between 0 and 1
...
if __name__ == '__main__':
    ...
    method = opt.method
    model = model_generator(method, pretrained_model_path).to(device)  # Use selected device
